...
last_trans, rough_transform, rough_trajectory, smooth_trajectory, smooth_transform = [], [], [], [], []
...
while src.isOpened():
    ...
    cur_corner = cv2.goodFeaturesToTrack(prev_gray, 200, 0.01, 30)
    ...
    if trans is None:
        trans = last_trans
    last_trans = trans
    dx, dy, da = trans[0][2], trans[1][2], np.arctan2(trans[1][0], trans[0][0])*180/math.pi
...
rotonly = 0
for trans in smooth_transform:
    trans[2] = trans[2]*math.pi/180
    ret, cur = src.read()
    if not ret:
        break
